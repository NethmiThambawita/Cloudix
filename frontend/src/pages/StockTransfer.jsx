...
function StockTransfer() {
  ...
  const handleSubmit = async (values) => {
    ...
    if (!stock?.product?._id) {
      toast.error('Cannot transfer: product information is missing.');
      return;
    }
    ...
    if (values.quantity > stock.quantity) {
      toast.error(`Cannot transfer ${values.quantity} units. Only ${stock.quantity} units available.`);
      return;
    }
    ...
    try {
      ...
      toast.celebrate(
        ...
        `${values.quantity} units of ${stock.product.name} transferred from ${values.fromLocation} to ${values.toLocation}.`
      );
      navigate('/stock');
    } catch (error) {
      ...
      const errorMessage = error.response?.data?.message || 'Unable to complete the transfer. Please try again or contact support.';
      ...
      <Row gutter={16}>
        <Col xs={24} lg={12}>
          <Card title="Stock Information" style={{ marginBottom: 16 }}>
            ...
            {stock.product?.category && <p><strong>Category:</strong> {stock.product.category}</p>}
            ...
            <Form form={form} layout="vertical" onFinish={handleSubmit}>
              ...
              <Form.Item
                ...
                rules={[
                  ...
                  ({ getFieldValue }) => ({
                    validator(_, value) {
                      if (!value || getFieldValue('fromLocation') !== value) return Promise.resolve();
              ...
              >
                <Select placeholder="Select destination location">
                  {locations.filter(loc => loc !== stock.location).map(loc => <Option key={loc} value={loc}>{loc}</Option>)}
                </Select>
              </Form.Item>
              ...
              <Form.Item
                ...
                rules={[
                  ...
                  ({ getFieldValue }) => ({
                    validator(_, value) {
                      if (!value || value <= stock.quantity) return Promise.resolve();
                      return Promise.reject(new Error(`Cannot exceed available quantity (${stock.quantity})`));
                    }
                  })
                ]}
              >
                <InputNumber style={{ width: '100%' }} min={1} max={stock.quantity} placeholder="Enter quantity" />
              </Form.Item>
              ...
              <Form.Item label="Notes (Optional)" name="notes">
                <TextArea rows={3} placeholder="Additional notes about this transfer" />
              </Form.Item>

              <Form.Item>
                <Space>
                  <Button type="primary" htmlType="submit" icon={<SwapOutlined />} loading={transferring}>
                    Transfer Stock
                  </Button>
